  ...
  def bubble_sort!(&prc)
    ...
    until sorted
      ...
      (0...self.length).each do |i|
        if prc.call(self[i], self[i + 1]) == 1 && i != self.length - 1
  ...
  def bubble_sort(&prc)
    ...
    until sorted
      ...
      (0...new_array.length).each do |i|
        if prc.call(new_array[i], new_array[i + 1]) == 1 && i != new_array.length - 1
...
def substrings(string)
  array = []
  string.each_char.with_index do |ele1, i1|
    string.each_char.with_index do |ele2, i2|
      if i2 >= i1
        array << string[i1..i2]
      end
    end
...
class Array
  ...
  def my_inject(&blk)
    self.my_each.with_index do |ele, i|
      if i != self.length - 1
        self[i + 1] = blk.call(ele, self[i + 1])
      end
    end
    self[-1]
  end
...
#load 'prep_iteration_exercises.rb'